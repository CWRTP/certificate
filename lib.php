 ...
 */
// function certificate_cronlist()
// {
// 	global $CFG, $DB;

// 	$certificateModule = $DB->get_record_select('modules', "name = 'certificate'");
// 	return $DB->get_records_sql(
// 			"SELECT cm.id, cm.course, C.userid, cm.instance, C.mailed, C.delivery
// 			FROM {course_modules} cm
// 			JOIN(
// 			SELECT ci.certificateid, ci.userid, ci.certdate, ci.mailed, c.autogen, c.delivery
...
// 			AND c.autogen = 1
// 	)C
// 			ON cm.instance = C.certificateid
// 			WHERE cm.module=$certificateModule->id
// 			");
// }
...
// 			ON DK.userid = gg.userid
// 			WHERE DK.gradeitemid = gg.itemid
// 			AND rawgrade BETWEEN grademin AND grademax");
// }
/**
...
function certificate_cron () {
	...
	foreach($cronlist as $cron)
	{
		...
		// CEUs
		if($certificate->certificatetype=="letter_CEUs")
		{
		...
		if ($certificate->savecert == 1) {
			...
			$filename = clean_filename($certificate->name.'.pdf');
			certificate_save_pdf_cron($file_contents, $certrecord->id, $filename, $context->id, $user);
		}
 ...
 * @return null
 */
function certificate_email_teachers2($course, $certificate, $certrecord, $cm, $user) {
	global $CFG, $DB;
 ...
 * Code suggested by Eloy Lafuente
 */
function certificate_email_others2($course, $certificate, $certrecord, $cm, $user) {
	global $CFG, $DB;
 ...
 * @return bool
 */
function certificate_save_pdf_cron($pdf, $certrecordid, $filename, $contextid, $user) {
	global $DB;
 ...
 * @param stdClass $cm
 */
function certificate_issue2($course, $certificate, $certrecord, $cm, $user) {
	global $DB;
	...
	$date = certificate_generate_date2($certificate, $course, $user);
	$certrecord->certdate = $date;
	$DB->update_record('certificate_issues', $certrecord);
	certificate_email_teachers2($course, $certificate, $certrecord, $cm, $user);
	certificate_email_others2($course, $certificate, $certrecord, $cm, $user);
}
 ...
 * @return mixed
 */
function certificate_print_mod_grade2($course, $moduleid, $user) {
	global $DB;
 ...
 * @return string the date
 */
function certificate_generate_date2($certificate, $course, $user) {
	global $DB;
	...
	if ($certificate->printdate == '2') {
		...
		$modinfo = certificate_print_mod_grade2($course, $certificate->printdate, $user);
		$certdate = $modinfo->dategraded;
	} else {
		$certdate = time();
	}
